    ...
    stages {
        ...
        stage('Run Docker container') {
            steps {
                ...
                sh '''
                docker run -d --name php-app-prod -p 8088:80 $DOCKER_TAG
                '''
            }
        }
        ...
        stage('Deploy') {
            steps {
                script {
                    ...
                    def message = "Deployment of $DOCKER_TAG was successful by Name: MD HARUNUR RASHID and Email: harunur@ba-systems.com"
                    sh """
                    curl -H "Content-Type: application/json" -d '{ "content": "${message}" }' ${DISCORD_WEBHOOK_URL}
                    """
                }
